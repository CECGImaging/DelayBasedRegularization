...
inputDir = '../../ExampleData/ventricles';
outputDir = '../../Reconstructions/ventricles';
signalNames = {'rvSeptal'}; % {'lvLateral','rvLateral','rvSeptal'}
...
lcurveBounds = [-4 4];          % bounds of logLambda for sampling the L-curve
...
sensorNodes = [16,96,204,1108,295,1251,417,1593,883,1474,1658,547,2073,575]; % distributed all over epi- and endocardium
% sensorNodes = [22 43 70 104 147 198 254 307 359 410 454 496]; % line on anterior epicardium

%% General

load(sprintf('%s/geometry/heart.mat', inputDir));
mesh = PrepareTriangleMesh(heart.points, heart.cells);

load(sprintf('%s/reguMat/L_sigma7.mat', inputDir));

A = load(sprintf('%s/transferMat/transferTMV.mat', inputDir));
load(sprintf('%s/geometry/electrodes.mat', inputDir));
A = A.transferTMV(electrodes,:);
...
%%
for sn = 1:numel(signalNames)
signalName = signalNames{sn};

try

resultsDir = sprintf('%s/%s', outputDir, signalName);
...
load(sprintf('%s/signals/tmv_%s.mat', inputDir, signalName));
tmv = double([repmat(tmv(:,1),1,20) tmv(:,1:end-71)]);

% reference time interval for computing the noise power
% (timesteps without activity are excluded)
refTimeInterval = [21 find(abs(max(tmv,[],1)-max(tmv(:,end),[],1))>1, 1, 'last')];

B = A * tmv;
rng(1);
B = addwhitenoise(B, SNR, refTimeInterval);

%% Estimate true activation times and visualize and store the truth

actTimes_true = globalActTimes_estimate(tmv, pairs, actTimesMat, actTimesSigma, 'spatiotemporal', Gx, Gy, Gz);

fig = figure('Name','Truth', 'WindowStyle','docked');
delayRegu_visualize(actTimes_true, tmv, mesh, sensorNodes);
saveas(fig, sprintf('%s/truth.fig', figDir));

results.TMV_true = tmv;
...
factorsLambdaL = linspace(1,0,numIter).^10;
...
for i = 2:numIter
    ...
    % estimate activation times
    Xb = tmvCorrectFloatingBaseline(X);
    actTimes = globalActTimes_estimate(Xb, pairs, actTimesMat, actTimesSigma, 'spatiotemporal', Gx, Gy, Gz);
